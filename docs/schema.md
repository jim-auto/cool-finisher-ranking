...
`image` は任意です。設定しない場合、カードには `script.js` の `genreDefaultImages` で定義したジャンル別の既定画像が表示されます。既定画像も Pexels License など、利用条件を確認できるものだけを使います。
...
- 汎用のジャンル画像を増やす場合は、`script.js` の `genreDefaultImages` に追加する。
