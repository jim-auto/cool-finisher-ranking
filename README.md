...
画像を追加する場合は、ライセンス確認済みの画像だけを使います。特にアニメ、ゲーム、特撮の本編カットや公式画像は無断転載になりやすいため、許諾や利用条件が確認できないものは `image` に入れないでください。個別画像がないデータは、`script.js` の `genreDefaultImages` で定義した Pexels などの安全なジャンル別画像で表示されます。
